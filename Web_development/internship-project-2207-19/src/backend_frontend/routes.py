...
import sqlite3
import secrets
import pathlib # For Google signin
import datetime
import requests
import googlemaps
...
from google.oauth2 import id_token # For Google signin
from pip._vendor import cachecontrol # For Google signin
import google.auth.transport.requests # For Google signin
from google_auth_oauthlib.flow import Flow # For Google signin
...
flow = Flow.from_client_secrets_file(
    ...
    redirect_uri="http://localhost:5000/login/google/authorized" # also configured on my Google account
)
...
def callback():
    ...
    if not state or state != request.args.get("state"):
        abort(500)  # State does not match or missing!
    ...
    # Extract user needed credentials from session
    ...
    if user is None:
        user = User(username=session['name'], email=session['email'],password='12345', image_file='default.jpg')
        db.session.add(user)
        db.session.commit()
...
def submit_workspace():
    ...
    if response.status_code == 200:
        ...
        flash('Error while processing the address', 'danger')
        return redirect(url_for('add_workspace'))
